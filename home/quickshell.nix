  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.enable {
      home = {
        ...
        file.".local/state/quickshell/.keep".text = "";

        # colors.json is written at runtime by matugen (the quickshell-colors
        # template is wired into config.toml from home/stivce.nix) and watched
        # live by Colors.qml, so home-manager must not own it as a read-only
        # store symlink. Seed the checked-in default exactly once; after that
        # matugen owns it. Mirrors seedMatugenDefaults in home/stivce.nix.
        activation.seedQuickshellColors = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      ...
      systemd.user = {
        services = {
          ...
          # Keep clipboard history in the per-login runtime directory. This
          # preserves the launcher's history within a session without writing
          # copied passwords, tokens, or images to persistent storage.
          cliphist-text = {
            Unit = {
            ...
            Service = {
              ExecStart = "${pkgs.wl-clipboard}/bin/wl-paste --type text --watch ${pkgs.cliphist}/bin/cliphist -db-path %t/cliphist/db -max-items 200 store";
              Restart = "on-failure";
              RestartSec = 2;
            };
            Install.WantedBy = [ "graphical-session.target" ];
          ...
          cliphist-image = {
            ...
            Service = {
              ExecStart = "${pkgs.wl-clipboard}/bin/wl-paste --type image --watch ${pkgs.cliphist}/bin/cliphist -db-path %t/cliphist/db -max-items 200 store";
              Restart = "on-failure";
              RestartSec = 2;
            };
            Install.WantedBy = [ "graphical-session.target" ];
        ...
        # Both watcher services share this private volatile database directory.
        tmpfiles.rules = [ "d %t/cliphist 0700 - -" ];
      };
    })
  ];
}
